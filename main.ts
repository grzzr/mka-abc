...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    let vx, vy = 0
direcao == 'norte'? (vy=-120):
    direcao == 'sul'? (vy=120):
    direcao == 'leste'? (vx=120):
    direcao == 'oeste'? (vx=-120): null
...
function criarInimigo () {
    inimigo = sprites.create(assets.image`inimigo`, SpriteKind.Enemy)
    inimigo.follow(jogador, 30)
    inimigo.setPosition(randint(0, scene.screenWidth()), randint(0, 50))
